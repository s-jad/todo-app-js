import {
  compareAsc, compareDesc, isSameDay, isAfter, parse, isValid, format,
} from 'date-fns';
...
const Display = ((doc) => {
  ...
  const getCurrentProjectGrid = () => state.currentProjectGrid;

  const getProgressBarPercentages = () => state.progressBarPercentages;

  const getProgressBarCount = () => state.progressBarCount;
  ...
  const renderWelcomeScreen = () => {
    ...
    if (userJson !== null && userJson !== 'undefined') {
      prevUser = JSON.parse(win.localStorage.getItem('user'));
    }
    ...
    welcomeInput.addEventListener('input', (ev) => {
      if (ev.target.value === '') {
        welcomeConfirmBtn.disabled = true;
      } else {
  ...
  const getShrinkingAnimation = (projectCard) => {
    ...
    pc.removeChild(projectTodoListContainerOuter);

    pc.classList.remove('expanded');
    pc.classList.add('shrinking');
    const gridBorderWidth = 0;
    const cardBorderWidth = 1;
    pc.style.top = `${currentY + cardBorderWidth + gridBorderWidth}px`;
    pc.style.left = `${currentX + cardBorderWidth + gridBorderWidth}px`;
    ...
    pc.style.animation = `${animationName} ${animationDuration} ${animationTransition}`;
  ...
  const renderShrunkProject = (ev) => {
    ...
    if (ev instanceof win.MouseEvent) {
    ...
    const handleShrinkingAnimationEnd = () => {
      ...
      for (let i = 0; i < progressBars.length; i++) {
        const progressBarIdNum = parseInt(progressBars[i].id.slice(progressBars[i].id.lastIndexOf('-') + 1), 10);
      ...
      deleteBtns.forEach((btn) => {
        btn.addEventListener('click', (btnEv) => {
          ...
          renderDeleteProjectModal(btnEv);
        });
      });
      ...
      projectsInGrid.forEach((proj) => {
        ...
        proj.addEventListener('searchGrow', (growEv) => {
          renderExpandedProject(growEv);
        });

        proj.addEventListener('searchShrink', (shrinkEv) => {
          renderShrunkProject(shrinkEv);
        });
  ...
  const getExpansionAnimation = (projectCard, projectGrid, gridRect) => {
    const pc = projectCard;
    const cardStyle = win.getComputedStyle(projectCard);
    const gridStyle = win.getComputedStyle(projectGrid);
    ...
    state.expandedCardPrevPosition = {
  ...
  const handleCheckBoxProjectGrid = (ev) => {
    ...
    const todoIndex = target.name.slice(target.name.lastIndexOf('-') + 1);
    ...
    const checkBoxContainer = target.parentNode;
    ...
    const progressbarIdNum = parseInt((progressBar.id).slice(progressBar.id.lastIndexOf('-') + 1), 10);
  ...
  const handleTodoFieldInputEvent = (ev, fieldType) => {
    ev.stopPropagation();
    const parent = ev.target.parentNode;
    const sibling = parent.querySelector('p');

    if (ev.target.value !== '') {
      sibling.innerText = `${ev.target.value}`;
      postTodoUpdatesToUser(fieldType, ev.target.value);
    }
  ...
  const handleTodoNameInputEvent = (currentTNI, todoNames, todoNameInput, todoInputContainer) => {
    ...
    const todoPresent = todoNames.findIndex((existingTodoName) => existingTodoName === todoName);
    if (todoPresent !== -1) {
  ...
  const handleTodoPriorityInputEvent = (ev, parent) => {
    ...
    if (userInput === '') {
      ...
      return false;
    } if (parseInt(userInput, 10) < parseInt(ev.target.min, 10)) {
      const existingPriorityWarning = doc.querySelector('[id^="warn-priority-"]');
      if (existingPriorityWarning !== null) {
        parent.removeChild(existingPriorityWarning);
      }
      ...
      return false;
    } if (parseInt(userInput, 10) > parseInt(ev.target.max, 10)) {
      const existingPriorityWarning = doc.querySelector('[id^="warn-priority-"]');
      if (existingPriorityWarning !== null) {
        parent.removeChild(existingPriorityWarning);
      }
  ...
  const handleTodoInfoFieldEvent = (target) => {
    ...
    const { fieldType } = target.dataset;
  ...
  const renderExpandedTodo = (ev) => {
    ...
    const checkboxTabIndex = parseInt(todoContainer.querySelector('.todo-check').tabIndex, 10);

    const todoInfo = generateExpandedTodoInfo(todoToExpand, checkboxTabIndex);

    todoContainer.classList.add('expanded');

    const otherTodos = Array.from(todoContainer.parentNode.childNodes)
      .filter((node) => node.nodeType === 1 && node !== todoContainer);

    otherTodos.forEach((todo) => {
      const t = todo;
      t.classList.add('shrink');
      t.style.border = 'transparent';
  ...
  const handleTodoExpansion = (ev) => {
    ...
    if ((todoContainer.classList.contains('expanded')
            && ev instanceof win.MouseEvent)
            || (todoContainer.classList.contains('expanded')
            && ev instanceof win.KeyboardEvent)
    ) {
      ...
      otherTodos.forEach((todo) => {
        setTimeout(() => {
          ...
          t.style.visibility = '';
          setTimeout(() => {
            t.style.border = '1px solid var(--contrast-orange-glow)';
  ...
  const renderTodosForProjectExpansion = (todosToRender, projectTodoList) => {
    todosToRender.forEach((todo, index) => {
      ...
      todoContainer.addEventListener('keypress', (ev) => {
        if (ev.key === 'Enter') {
          handleTodoExpansion(ev);
        }
      });

      todoContainer.addEventListener('searchTodoGrow', (ev) => {
        handleTodoExpansion(ev);
      });
  ...
  const scrollProjects = (ev) => {
    ...
    const focussedProject = allProjects.find((proj) => proj === doc.activeElement);
  ...
  const switchToProjectGrid = () => {
    ...
    win.addEventListener('keypress', scrollProjects);
  ...
  const scrollTodosLeft = () => {
    ...
    const visibleTodo = todos.find((todo) => !todo.classList.contains('hide-left')
                && !todo.classList.contains('hide-right'));

    const visibleTodoIdNum = parseInt(visibleTodo.id.match(/\d+/g), 10);
    ...
    const nextTodo = todos.find((todo) => parseInt(todo.id.match(/\d+/g), 10) + 1 === visibleTodoIdNum);
  ...
  const scrollTodosRight = () => {
    ...
    const visibleTodoIdNum = parseInt(visibleTodo.id.match(/\d+/g), 10);
    const maxNumTodos = parseInt(doc.querySelector('#create-project-todo-count-input').value, 10);
  ...
  const handleModalInputChanges = () => {
    ...
    setTimeout(() => {
      ...
      if (uniqueProjectName !== -1
                || parseInt(projectTodoCount.value, 10) > parseInt(projectTodoCount.max, 10)
  ...
  const renderTodoContainers = (
    ...
    createProjectTodoCountInput,
  ) => {
    todoCount = parseInt(ev.target.value, 10);
    const todoNames = [];
  ...
  const renderCreateNewProjectModal = () => {
    ...
    createProjectModalContainer.innerHTML = `
        <form action="#" id="create-project-form" method="POST">
          ...
          <label for="create-project-todo-count" class="create-project-label">
            <span class="warn-hl">*</span> Number of todos:</label>
    ...
    win.removeEventListener('keypress', scrollProjects);
  ...
  const handleCheckBoxTodoList = (ev) => {
    const { target } = ev;
    const todoRow = target.parentNode.parentNode.parentNode;
    ...
    project.todos[todoIndex].checked = target.checked;
  };
  ...
  const generateSwitchViewBtn = () => {
    ...
    actualBtn.addEventListener('closeViewBtn', () => {
      eyeComponents.forEach((component) => {
  ...
  const renderNewProject = (project) => {
    ...
    actualDeleteBtn.addEventListener('click', (ev) => {
      ev.stopPropagation();
    ...
    newProjectCard.addEventListener('searchGrow', (ev) => {
      renderExpandedProject(ev);
    });

    newProjectCard.addEventListener('searchShrink', (ev) => {
      renderShrunkProject(ev);
    });

    newProjectCard.addEventListener('addProjectShrink', (ev) => {
      renderShrunkProject(ev);
    });
  ...
  const renderProjectDashboard = (username, prevUser) => {
    ...
    const projectGridOuter = renderProjectGrid();
    const projectGrid = projectGridOuter.querySelector('#project-grid');
    ...
    const progressBarCount = JSON.parse(win.localStorage.getItem('progressBarCount'));
    state.progressBarCount = progressBarCount;

    const progressBarPercentages = JSON.parse(win.localStorage.getItem('progressBarPercentages'));
  ...
  const removeProjectFromProjectGrid = (projectTitle) => {
    ...
    if (projectToRemove.classList.contains('expanded')) {
      ...
      const storedProjectGrid = state.currentProjectGrid;

      const projectToRemoveInGrid = storedProjectGrid.querySelector(`#${projectCardId}`);
      storedProjectGrid.removeChild(projectToRemoveInGrid);

      const otherProjects = Array.from(storedProjectGrid.querySelectorAll('[id^="project-card-"]'));
      const deleteBtns = Array.from(storedProjectGrid.querySelectorAll('.invisible-btn'));
      ...
      dashboardContainer.appendChild(storedProjectGrid);
    } else {
