...
const SearchBar = ((doc) => {
  ...
  const displayProjectMatches = (wordToMatch) => {
    ...
    allProjects.forEach((proj) => {
      ...
      const match = matches.some((m) => {
        const matchTitle = m.querySelector('.project-title').innerText.trim();
  ...
  const findTodoMatches = (wordToMatch) => {
    ...
    if (wordToMatch === '') {
      projects.forEach((proj) => {
        const p = proj;
        p.classList.remove('search-invisible');
  ...
  const displayTodoMatches = (wordToMatch) => {
    ...
    if (projectMatches.length === 1) {
      ...
      state.expandedMatchingProject = true;
      [state.currentlyExpandedProject] = projectMatches;
    ...
    allProjects.forEach((proj) => {
      ...
      const match = projectMatches.some((projMatch) => {
        const matchTitle = projMatch.querySelector('.project-title').innerText.trim();
  ...
  const generateSearchBar = () => {
    ...
    searchBarContainer.addEventListener('keypress', (ev) => {
